...
that underlie the API design, including Wikidata as a representative example of
a Linked Data knowledge graph. Section~\ref{REL_GEONAMES} describes GeoNames
and Apache Solr, which power the city search functionality.
Section~\ref{REL_WL} describes the Walter-Lieth climatogram.
...
\begin{sidewaystable}[p]
  ...
  \begin{tabular}{lcccccc}
    ...
    \midrule
    Scope                    & World  & World  & World  & World  & World  & World \\
    Spatial resolution & 30\,min & station & 1\,km & varies & 0.1$^\circ$ & 30\,sec \\
    Climate periods          & 1 & 1 & 1 & \xmark & \cmark & 5 \\
    Weather periods          & \xmark & \xmark & \cmark & \cmark & \cmark & 1951--2024 \\
    Source data              & Stations & Stations & ERA5T & Satellite & ERA5 & WorldClim \\
    \bottomrule
  \end{tabular}
  \normalsize
\end{sidewaystable}
...
temperature (\texttt{tmin}), maximum temperature (\texttt{tmax}), average
temperature (\texttt{tavg}), precipitation (\texttt{prec}), solar radiation
(\texttt{srad}), wind speed (\texttt{wind}), and vapour pressure
(\texttt{vapr}).

WorldClim distributes the raw data as GeoTIFF raster files. The complete archive
consists of 8{,}836 files totalling approximately 85.6\,GB, which requires GIS
software or programming libraries such as GDAL or rasterio to process ---
putting the data out of reach for non-specialist users.
...
In addition to the seven monthly variables, WorldClim provides 19 yearly
bioclimatic variables (BIO1--BIO19) derived algorithmically from the monthly
temperature and precipitation series. These capture biologically meaningful
aspects of climate such as annual mean temperature (BIO1), annual precipitation
(BIO12), and temperature seasonality (BIO4), and are widely used as predictors
in species distribution models and ecological niche analyses. A full description
of all 19 variables is available at the WorldClim
website\footnote{\url{https://www.worldclim.org/data/bioclim.html}}.
...
From the CRU-TS-based weather rasters, additional 30-year climate normals for
the periods 1951--1980, 1961--1990, 1981--2010, and 1991--2020 were prepared
by Vega Gorgojo~\cite{scrapi} and made available through SCRAPI. Together with
the original 1970--2000 WorldClim baseline, this gives five overlapping 30-year
periods that can be compared to detect long-term trends.

\subsection{A Virtual Semantic API for WorldClim}\label{REL_WORLDCLIM_API}

To make WorldClim data accessible over the web without requiring Geographic
Information System (GIS) expertise, Guillermo Vega Gorgojo at the Universidad
de Valladolid developed the WorldClim Virtual Semantic API~\cite{scrapi},
hereafter referred to as SCRAPI. The design of SCRAPI is motivated by a
scalability problem inherent in the materialisation approach to semantic spatial
data.

Even for GIS experts familiar with libraries such as
GDAL~\cite{gdal}~\cite{rasterio} or rasterio, working directly with the
WorldClim archive is impractical: the complete dataset comprises over 8{,}836
raster files totalling approximately 85.6\,GB, each requiring individual
loading, reprojection, and extraction before any query can be answered. Yet
climate and weather data of this kind are increasingly needed across multiple
domains --- forestry, ecology, agriculture, and climate change research ---
by users who are domain specialists rather than GIS programmers. Semantic Web
(SW) technologies, described in Section~\ref{REL_SEMWEB}, are specifically
designed to address this kind of data integration challenge by exposing
heterogeneous datasets through a common, web-friendly query interface.
...
in advance. Queries are answered at request time by the GDAL
library~\cite{gdal} reading directly from the GeoTIFF files, while the API
surface is identical to what a fully materialised triplestore would expose.
The result is a lightweight, fast, and scalable Representational State Transfer
(REST) service that answers climatic and weather queries at any available
resolution for any location on Earth.
...
\begin{itemize}
  ...
  \item \textbf{Cell.} A single rectangular tile within a grid. A cell is
    ...
    Well-Known Text (WKT) format~\cite{ogc_wkt}, a centroid, an elevation value, 
    and the IRIs of all pixels located within it.
...
\texttt{avgpixelvaluesinpolygonGEO} queries aggregate pixel values into a 
single average per raster and month.
...
For example, the following two triples are part of the Wikidata knowledge graph
and encode the fact that Madrid is a city with coordinates $40.4^\circ$\,N,
$3.7^\circ$\,W:
\begin{center}
...
\begin{lstlisting}[language=SPARQL, caption={A SPARQL query over the Wikidata
  graph retrieving all instances of human settlement.},
...
described in Section~\ref{REL_SEMWEB_WIKIDATA}.
...
For example, the Wikidata IRI for Madrid ---
\url{https://www.wikidata.org/entity/Q2807} --- returns a structured RDF
description of the city when dereferenced, including its coordinates, population,
and links to related entities. Any dataset that wishes to refer to Madrid can
reuse this IRI, creating a link between otherwise independent data sources.

SCRAPI builds on the same principles by assigning stable HTTP IRIs to every
resource in the WorldClim data model --- every grid, cell, raster, and pixel.
Although these IRIs are not yet dereferenceable as RDF, their structure encodes
the key properties of each resource. For example:
\begin{center}
  \texttt{http://climate.gsic.uva.es/data/Pixel\_2.5m\_r1151c4211\_prec\_w1953}
\end{center}
encodes the grid resolution (\texttt{2.5m}), the cell row and column
(\texttt{r1151c4211}), the climate variable (\texttt{prec}), and the time
period (\texttt{w1953}). Climatica exploits this encoding in its heatmap
engine: rather than making a separate API call to retrieve the geographic bounds
of each pixel, the application parses the IRI directly to reconstruct the row
and column indices and compute the bounding box from the known cell dimensions
of the grid.
...
SCRAPI is accompanied by the RASter Ontology (RASO)~\cite{raso}, which defines
the resource types used in the WorldClim data model --- Grid, Cell, Raster, and
Pixel --- together with their properties and relationships. RASO provides the
vocabulary that makes the IRI structure of SCRAPI resources meaningful and
stable.

\subsection{Wikidata as a Linked Data Knowledge Graph}\label{REL_SEMWEB_WIKIDATA}

Wikidata~\cite{wikidata} is a prominent example of a cross-domain knowledge
graph that follows Semantic Web standards in practice. Maintained by the
Wikimedia Foundation, it stores structured data as subject--predicate--object
statements, with subjects and objects identified by stable HTTP IRIs
(Q-identifiers such as \texttt{Q2807} for Madrid) and predicates by
P-identifiers. As of 2025, Wikidata contains over 100 million items in hundreds
of languages, covering people, places, organisations, and more.
...
with coordinates, administrative hierarchies, and population figures. Its open
licence (CC0) and stable HTTP IRIs make it an attractive foundation for
\textbf{reverse geocoding} --- resolving a latitude/longitude pair to a named
city --- without relying on proprietary geocoding services. Wikidata exposes a
GeoSearch endpoint (\texttt{list=geosearch}) that returns entities within a
given radius of a coordinate pair, and a \texttt{wbgetentities} endpoint that
fetches multilingual labels for any entity by identifier.
...
GeoNames~\cite{geonames} is a free geographical database available under a
Creative Commons Attribution licence. Its primary distribution file,
\texttt{allCountries.txt}, contains over 12~million records covering populated
places, administrative regions, natural features, and points of interest,
each annotated with coordinates, feature class, country code, population,
and elevation. A companion file, \texttt{alternateNamesV2.txt}, provides
localised names for each record in hundreds of languages, making GeoNames
the most comprehensive open multilingual gazetteer available.
...
\begin{itemize}
  ...
  \item \textbf{Humid periods} --- months where precipitation / 2 $>$
    temperature --- are shown in a cool colour (typically blue). They indicate
    surplus water.
\end{itemize}

Figure~\ref{fig:wl_example} shows a Walter-Lieth climatogram for Valladolid
generated by Climatica. The large yellow area spanning the summer months
confirms the semi-arid character of the site, while the blue areas in winter
and spring indicate a surplus of precipitation over evapotranspiration demand.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.6\textwidth]{Images/example-walter-lieth-diagram.png}
  \caption{Walter-Lieth climatogram for Valladolid (climate dataset,
    1991--2020). Yellow areas indicate arid months; blue areas indicate
    humid months.}
  \label{fig:wl_example}
\end{figure}

% * ─── 2.6 Summary ──────────────────────────────────────────────────────────────
\section{Summary}\label{REL_SUMMARY}

This chapter has surveyed the landscape in which Climatica was developed.
The review of existing climate data applications revealed a clear gap: no
publicly available tool combines ease of use, open kilometre-scale raster data,
interactive multi-view exploration, and support for temporal trend analysis in a
single registration-free browser interface. The tools closest to Climatica's
objectives --- ClimateCharts.net and Climate-Data.org --- are accessible but
static, while the most powerful platforms --- NASA Giovanni and Copernicus C3S
--- require specialist expertise that puts them out of reach for non-specialist
users.

The remaining sections introduced the foundational technologies on which
Climatica is built. WorldClim provides the historical climate and meteorological
dataset, exposed through SCRAPI --- a virtual semantic API that avoids the
scalability problems of full RDF materialisation. GeoNames and Apache Solr
provide the city search infrastructure, offering fast multilingual partial-word
matching across 12~million populated places. Wikidata serves as a reverse
geocoding service for coordinate-based lookups. The Semantic Web and Linked Data
principles underlie both SCRAPI and the Wikidata integration. Finally, the
Walter-Lieth climatogram provides the primary visualisation convention adopted
by Climatica for communicating the seasonal water balance of a location. Chapter
3 describes how these components were combined into a working application.